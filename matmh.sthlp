...
{p 8 8 2}
{bf:matmh} [{it:varlist}] [{it:if}] [{it:in}] [{it:weight}] [using {it:filename}] [, {it:options}]
...
{p 4 4 2}
{bf:matmh}, based on esttab, can output summary statistics to Stata interface, Word as well as LaTeX. User can use this command easily due to its simple syntax. It is worth noting that this command can only be used in version 15.1 or later.

{p 4 4 2}
Users can also append the output from {bf:matmh} to a existed word or LaTeX document,
...
{p 8 8 2}. {stata matmh}{p_end}

{p 4 4 2}Output summary statistics of specified variables{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight}{p_end}

{p 4 4 2}Output summary statistics of specified statistics and numeric format{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight, s(N sd(%9.3f) min(%9.2f) max(%9.2f))}{p_end}

{p 4 4 2}Add a custom title to the table{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight, ti(This is a custom title)}{p_end}

{p 4 4 2}Output the result to a .rtf file{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight using Myfile.rtf, replace}{p_end}

{p 4 4 2}Output the result to a .tex file{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight using Myfile.tex, replace}{p_end}

{p 4 4 2}Format table column in LaTeX to decimal point alignment{p_end}
{p 8 8 2}. {stata matmh price rep78 foreign weight using Myfile.tex, replace a(dot)}{p_end}
